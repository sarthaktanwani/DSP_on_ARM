...
void recursive_moving_average(float32_t *sig_src_arr, float32_t *sig_dest_arr, uint32_t sig_len, uint32_t filter_pts);
...
int main()
{
	...
	recursive_moving_average(&inputSignal_f32_1kHz_15kHz[0], &moving_avg_output[0], SIG_LEN, FILT_PTS);
...
void recursive_moving_average(float32_t *sig_src_arr, float32_t *sig_dest_arr, uint32_t sig_len, uint32_t filter_pts)
{
	...
	sig_dest_arr[(filter_pts - 1)/2] = acc/filter_pts;
	for(i = ceil(filter_pts/2); i < (sig_len - filter_pts/2) - 1; i++)
	{
		acc = acc + sig_src_arr[i + (filter_pts - 1)/2] - sig_src_arr[i - (filter_pts + 1)/2];
		sig_dest_arr[i] = acc/filter_pts;
	}
}
